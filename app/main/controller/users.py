...
import requests
from ..service.users import post_login, social_signin, post_signup, get_find_id, edit_temp_pw, post_email_check
...
@api.route('/check')
class PostEmailCheck(Resource):
  def post(self):
    """Post User Email Duplicate Check"""
    data = request.get_json().get('users')
    return post_email_check(data)
